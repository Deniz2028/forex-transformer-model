...
def calculate_mcc(y_true, y_pred):
    ...
    try:
        return matthews_corrcoef(y_true.cpu().numpy() if hasattr(y_true, 'cpu') else y_true, 
...
class HybridForexTrainer:
    ...
    def train_pair_model(self, pair_name: str, X: torch.Tensor, y: torch.Tensor, 
        ...
        for epoch in range(self.epochs):
            # 🆕 Training phase - artık MCC de döndürüyor
            train_loss, train_acc, train_mcc = self._train_epoch(train_loader, epoch)
            
            # 🆕 Validation phase - artık MCC de döndürüyor  
            val_loss, val_acc, val_mcc = self._validate_epoch(val_loader, epoch)
            ...
            history['train_mcc'].append(train_mcc)  # YENİ
            history['val_mcc'].append(val_mcc)      # YENİ
            ...
            if val_acc > best_val_acc:
             ...
             # 🆕 Enhanced Progress logging with MCC
            if epoch % 5 == 0 or epoch == self.epochs - 1:
                gap = train_acc - val_acc
                mcc_gap = train_mcc - val_mcc
                self.logger.info(
                    f"   📊 Epoch {epoch+1:3d}/{self.epochs}: "
                    f"Train: {train_acc:.1f}% (MCC: {train_mcc:.3f}) | "
                    f"Val: {val_acc:.1f}% (MCC: {val_mcc:.3f}) | "
                    f"Gap: {gap:+.1f}pp (MCC: {mcc_gap:+.3f}) | LR: {current_lr:.6f}"
                )
        ...
        final_train_mcc = history['train_mcc'][-1] if history['train_mcc'] else 0.0
        final_val_mcc = history['val_mcc'][-1] if history['val_mcc'] else 0.0

        self.logger.info(f"🎉 Training completed for {pair_name}")
        self.logger.info(f"   🏆 Best validation accuracy: {best_val_acc:.1f}%")
        self.logger.info(f"   🎯 Best validation MCC: {best_val_mcc:.3f}")
        ...
        self.logger.info(f"   🔍 MCC overfitting gap: {final_train_mcc - final_val_mcc:+.3f}")
        
        return self.model, history

    def _train_epoch(self, train_loader, epoch):
        """Single training epoch with gradient accumulation, mixed precision and MCC tracking"""
        self.model.train()
        total_loss = 0.0
        correct = 0
        total = 0
        
        # MCC için predictions ve targets topla
        all_predictions = []
        all_targets = []
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            # MCC için predictions ve targets sakla
        ...
        # MCC hesapla
        epoch_mcc = calculate_mcc(all_targets, all_predictions)
        
        return epoch_loss, epoch_acc, epoch_mcc

    def _validate_epoch(self, val_loader, epoch):
        """Single validation epoch with MCC calculation"""
        self.model.eval()
        total_loss = 0.0
        correct = 0
        total = 0
    ...
    def setup_loss_function(self):
        """Loss function konfigürasyonu (yeni config yapısı uyumlu)"""
        # loss_type değerini farklı konumlardan kontrol et
        # Loss type override kontrolü
        loss_type = None
        if hasattr(self.config, 'loss_type'):
            loss_type = self.config.loss_type
        elif hasattr(self.config, 'training') and hasattr(self.config.training, 'loss_type'):
            loss_type = self.config.training.loss_type
        elif hasattr(self.opt_config, 'loss_type'):
            loss_type = self.opt_config.loss_type
        else:
            loss_type = 'bce'  # güvenli varsayılan
        ...
        self.logger.info(f"✅ Loss function initialized: {self.criterion.__class__.__name__}")
