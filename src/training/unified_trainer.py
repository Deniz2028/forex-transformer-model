...
class UnifiedTrainer:
    ...
    def _train_lstm_real(self, model_name: str, model, X_train, y_train, X_val, y_val, 
        ...
        """Real LSTM training implementation with FIXED loss function"""
        ...
        for epoch in range(epochs):
            ...
            for batch_X, batch_y in train_loader:
                ...
                if use_sigmoid:
                    # BCEWithLogitsLoss: single output, float targets
                    if pred.dim() > 1:
                        pred = pred.squeeze(-1)  # Remove last dimension if needed
                    batch_y = batch_y.float()
                else:
                    # CrossEntropyLoss: multiple outputs, long targets
            ...
            if X_val is not None and y_val is not None:
                ...
                with torch.no_grad():
                    ...
                    # 🔧 FIXED: Proper validation loss and accuracy calculation
                    if use_sigmoid:
                        # BCEWithLogitsLoss validation
                        if val_pred.dim() > 1:
    ...
    def _train_transformer_real(self, model_name: str, model, X_train, y_train, X_val, y_val,
                            epochs, batch_size, learning_rate, weight_decay, patience):
        """Real Enhanced Transformer training with FIXED loss function"""
