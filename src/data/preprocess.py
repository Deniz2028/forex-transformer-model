...
class PairSpecificPreprocessor:
    ...
    def create_pair_specific_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 🔧 FIX: Add multi-timeframe features FIRST to create trend_strength
        df = self.create_multi_timeframe_features(df)
        
        # Pair-specific enhancements - NOW trend_strength is available
        if self.pair_name == 'EUR_USD':
            ...
            # 🔧 FIX: trend_strength check before using
            if 'trend_strength' in df.columns:
                df['gbp_usd_trend_persistence'] = df['trend_strength'].rolling(5).mean()
            else:
                # Fallback: create trend_strength from available features
                df['trend_strength'] = (
            ...
            try:
                ...
                df['aud_usd_commodity_proxy'] = 0.0  # Fallback
        ...
        # 🔧 REMOVED: Multi-timeframe features call (already done above)
        ...
        if self.target_mode == 'three_class':
            ...
            # 🔧 FIX: Ensure trend_strength exists
            ...
            df['trend_score'] = (
                ...
                (df['macd'] > df['macd_signal']).astype(int) +
                (df['bb_position'] > 0.5).astype(int) +
                df.get('trend_alignment_all', pd.Series(0, index=df.index)).astype(int)
            ) / 5
            ...
            # Directional strength - use safe column access
            momentum_alignment = df.get('momentum_alignment', pd.Series(0.5, index=df.index))
    ...
    # Target Fix - src/data/preprocess.py create_sequences fonksiyonu güncellemesi

    def create_sequences(self, features: pd.DataFrame, target: pd.Series) -> Tuple[np.ndarray, np.ndarray]:
        """Create sequences with FIXED target validation - PDF optimized."""
        ...
        # Apply normalization ONLY to features
        print(f"   📏 Applying z-score normalization (PDF optimization)...")
        ...
        if self.use_smote:
            ...
            # 🔧 TARGET FIX: Post-SMOTE target cleaning
            print(f"   🔧 Post-SMOTE target validation...")
            
            # Remove NaN/Inf values from SMOTE
            valid_mask = np.isfinite(target_array)
            if not np.all(valid_mask):
                print(f"   ⚠️ Removing {np.sum(~valid_mask)} invalid targets after SMOTE")
                features_array = features_array[valid_mask]
                target_array = target_array[valid_mask]
            
            # Force targets to valid range
            if self.target_mode == 'binary':
                target_array = np.clip(target_array, 0, 1)
                target_array = np.round(target_array).astype(int)
            else:  # three_class
                target_array = np.clip(target_array, 0, 2)
                target_array = np.round(target_array).astype(int)
            
            print(f"   ✅ Post-SMOTE target unique: {np.unique(target_array)}")
        
        # Create sequences with TARGET VALIDATION
        sequences = []
        targets = []
        
        print(f"   🔧 Creating sequences with enhanced target validation...")
        skipped_invalid = 0
        ...
        for i in range(self.sequence_length, len(features_array)):
            ...
            # 🔧 TARGET VALIDATION - GPU FIX
            # Check for NaN/Inf
            if np.isnan(tgt) or np.isinf(tgt):
                skipped_invalid += 1
                continue
            
            # Force to valid range and type
            if self.target_mode == 'binary':
                tgt = int(np.clip(tgt, 0, 1))
                if tgt not in [0, 1]:
                    skipped_invalid += 1
                    continue
            else:  # three_class
                tgt = int(np.clip(tgt, 0, 2))
                if tgt not in [0, 1, 2]:
                    skipped_invalid += 1
                    continue
        ...
        if skipped_invalid > 0:
            print(f"   ⚠️ Skipped {skipped_invalid} invalid targets during sequence creation")
        
        # Convert to arrays with proper dtypes
        X = np.array(sequences, dtype=np.float32)
        y = np.array(targets, dtype=np.int64)  # Important: int64 for PyTorch CrossEntropy
        
        print(f"   📊 Final tensor shapes: X={X.shape}, y={y.shape}")
        
        # 🔧 FINAL VALIDATION
        print(f"   🔧 Final target validation...")
        unique_targets = np.unique(y)
        print(f"   ✅ Final target unique: {unique_targets}")
        print(f"   ✅ Final target dtype: {y.dtype}")
        
        # Validation assertions
        assert y.dtype in [np.int32, np.int64], f"Target dtype must be integer, got {y.dtype}"
        assert not np.any(np.isnan(y)), "Target contains NaN values"
        assert not np.any(np.isinf(y)), "Target contains Inf values"
        
        if self.target_mode == 'binary':
            assert np.all((y >= 0) & (y <= 1)), f"Binary targets out of range: {unique_targets}"
            expected_classes = {0, 1}
        else:  # three_class
            assert np.all((y >= 0) & (y <= 2)), f"Three-class targets out of range: {unique_targets}"
            expected_classes = {0, 1, 2}
        
        actual_classes = set(unique_targets)
        if not actual_classes.issubset(expected_classes):
            raise ValueError(f"Invalid target classes: {actual_classes}, expected: {expected_classes}")
        
        print(f"   ✅ All target validations passed!")
        
        # Final distribution
        if self.target_mode == 'binary':
            unique, counts = np.unique(y, return_counts=True)
            final_dist = {int(k): int(v) for k, v in zip(unique, counts)}
            print(f"   🎯 Final binary distribution: {final_dist}")
            print(f"   🎯 Final minority ratio: {min(counts) / len(y):.3f}")
        else:
            unique, counts = np.unique(y, return_counts=True)
            final_dist = {int(k): int(v) for k, v in zip(unique, counts)}
            class_names = {0: 'Long', 1: 'Flat', 2: 'Short'}
            class_str = ', '.join([f"{class_names.get(k, k)}: {v}" for k, v in final_dist.items()])
            print(f"   🎯 Final class distribution: {class_str}")
