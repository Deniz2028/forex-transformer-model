...
class EnsembleTrainer:
        ...
        for i, config in enumerate(model_configs):
            ...
            try:
                ...
                if 'final_val_acc' in training_results:
                    ...
                    logger.info(f"   📊 Final Val Acc: {val_acc*100:.2f}%, Time: {train_time:.1f}s")
    ...
    def _get_training_params(self, config: Dict, model_index: int) -> Dict[str, Any]:
        """Get training parameters based on model type and diversity"""
        ...
        if model_type == 'lstm':
            params.update({
                'batch_size': 32 + model_index * 8,  # 32, 40, 48...
                'learning_rate': 0.001 * (0.8 ** model_index),  # Decreasing LR
            ...
            params.update({
                'batch_size': 32,
                'learning_rate': 0.0003 * (0.9 ** model_index),
                'patience': 8,
                'weight_decay': 0.01 + model_index * 0.005
            })
        
        elif model_type == 'hybrid_lstm_transformer':
            params.update({
                'batch_size': 24 + model_index * 4,  # Smaller batches for large model
                'learning_rate': 0.0005 * (0.85 ** model_index),
                'patience': 10,
                'weight_decay': 0.01
            })
        
        # Add diversity through different strategies
        diversity_strategies = [
            {'augmentation': True, 'label_smoothing': 0.1},
            {'augmentation': False, 'dropout_extra': 0.05},
            {'augmentation': True, 'early_stopping_metric': 'accuracy'}
        ]
        
        if model_index < len(diversity_strategies):
            params.update(diversity_strategies[model_index])
    ...
    def generate_diverse_configs(self, base_config: Dict, ensemble_models: List[str] = None, 
        ...
        for i, model_type in enumerate(model_types):
            ...
            if model_type == 'lstm':
                ...
                config.update({
                    'd_model': 128,             # ✅ Daha küçük, CLI ile uyumlu
                    'nhead': 8,
                    'num_layers': 2 + (i % 2),  # 2 veya 3 katman
                    'dropout': 0.1 + (i * 0.05),
                    'ff_dim': 256 + (i * 64)    # Hafif FFN katmanları
                })
