# ========================
# src/ensemble/ensemble_trainer.py - TAMAMEN DÜZELTİLMİŞ VERSİYON
# ========================
...
class EnsembleTrainer:
    """Enhanced Ensemble Trainer with real training integration - FIXED VERSION"""
    ...
    def calculate_model_weights(self, models: List[nn.Module], X_val: torch.Tensor, 
        ...
        """🔧 FIXED: Calculate performance-based model weights with tensor dimension handling"""
        ...
        for i, model in enumerate(models):
            ...
            with torch.no_grad():
                try:
                    # 🔧 FIX: Handle input dimension requirements for different models
                    model_type_name = type(model).__name__.lower()
                    
                    if 'lstm' in model_type_name or 'pair' in model_type_name:
                        # LSTM models need [batch, seq_len, features]
                        if X_val.dim() == 2:  # [batch, features]
                            model_input = X_val.unsqueeze(1)  # [batch, 1, features]
                        else:
                            model_input = X_val
                    else:
                        # Transformer and other models can handle [batch, features]
                        model_input = X_val
                    
                    logger.info(f"   📊 Model {i+1} ({model_type_name}): input shape {model_input.shape}")
                    
                    pred = model(model_input)
                    logger.info(f"   📊 Model {i+1} output shape: {pred.shape}")
                    
                    # 🔧 FIX: Handle different output formats properly
                    if pred.dim() == 2:
                        if pred.shape[1] == 2:  # [batch, 2] from LSTM CrossEntropy
                            # Convert to probabilities and take positive class
                            probs = torch.softmax(pred, dim=1)
                            pred_probs = probs[:, 1]  # Positive class probability
                        elif pred.shape[1] == 1:  # [batch, 1] from Transformer BCE
                    ...
                    logger.error(f"   📊 X_val shape: {X_val.shape}")
                    if 'model_input' in locals():
                        logger.error(f"   📊 Model input shape: {model_input.shape}")
...
# Model ağırlıklarını hesapla - ARTIK HATASIZ!
