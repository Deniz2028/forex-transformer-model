...
class UnifiedTrainer:
    ...
    def _train_lstm_real(self, model_name: str, model, X_train, y_train, X_val, y_val,
        ...
        for epoch in range(epochs):
            ...
            if X_val is not None and y_val is not None:
                ...
                # 🔧 FIX: Enhanced logging with validation accuracy
                if (epoch + 1) % 5 == 0:
                    current_lr = optimizer.param_groups[0]['lr']
                    logger.info(f"   📈 Epoch {epoch+1}/{epochs}: Train Loss: {avg_train_loss:.4f}, Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.2%}, LR: {current_lr:.6f}")
            else:
                # No validation data
                if (epoch + 1) % 5 == 0:
                    current_lr = optimizer.param_groups[0]['lr']
                    logger.info(f"   📈 Epoch {epoch+1}/{epochs}: Train Loss: {avg_train_loss:.4f}, LR: {current_lr:.6f}")
    ...
    def _train_transformer_real(self, model_name: str, model, X_train, y_train, X_val, y_val,
        ...
        for epoch in range(epochs):
            model.train()
            epoch_train_loss = 0.0
            train_correct = 0  # Accuracy için doğru tahmin sayacı
            train_total = 0    # Accuracy için toplam örnek sayacı
            ...
            avg_train_loss = epoch_train_loss / len(train_loader)
            train_acc = train_correct / train_total  # ✅ Accuracy hesapla
            history['train_loss'].append(avg_train_loss)
