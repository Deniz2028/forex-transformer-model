...
def add_common_args(parser):
    ...
    parser.add_argument(
        ...
        type=str, 
        default='lstm',
        choices=['lstm', 'transformer', 'enhanced_transformer', 'hybrid_lstm_transformer'],  # ← Bu eksikti
...
def train_ensemble_command(args):
    ...
    for pair_name, data in pair_data.items():
        ...
        base_config = config.get_model_config(args.model_type, args.target_mode)
        ...
        if args.ensemble_models:
            for i, model_type in enumerate(args.ensemble_models):
                if i < len(model_configs):
                    model_configs[i] = config.get_model_config(model_type, args.target_mode)
