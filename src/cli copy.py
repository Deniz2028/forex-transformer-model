...
def add_ensemble_args(parser):
    ...
    ensemble_group.add_argument('--ensemble_models', nargs='+',
                           choices=['lstm', 'pairspecificlstm', 'enhanced_transformer', 
                                   'hybrid_lstm_transformer', 'pdf_hybrid', 'transformer'],
...
# CLI.py içindeki train_ensemble_command fonksiyonunu tamamen değiştir

def train_ensemble_command(args):
    """Train ensemble of models - COMPLETELY FIXED"""
    ...
    pair_data = fetcher.fetch_all_pairs(
        ...
        count=cfg['data']['lookback_candles'], 
        pairs=args.pairs
    )
    ...
    for pair_name, data in pair_data.items():
        ...
        try:
            # Data preprocessing
            print(f"🔧 {pair_name} data preparation ({args.target_mode})...")
            
            # 🔧 FIXED: Correct parameter order for create_preprocessor
            preprocessor = create_preprocessor(pair_name, cfg)
            preprocessor.target_mode = args.target_mode
            
            features, target = preprocessor.prepare_pair_data(
                data, 
                dynamic_k_aggressive=getattr(args, 'dynamic_k_aggressive', True)
            )
            
            if features is None or len(features) < 1000:
                print(f"❌ {pair_name}: Insufficient clean data!")
                continue
            
            print(f"   🎯 {pair_name} features: {features.shape[1]}")
            print(f"   📊 Target distribution: {dict(zip(*np.unique(target, return_counts=True)))}")
            print(f"   ✅ {pair_name} clean data: {len(features)} records")
            ...
            if target.ndim > 1:
                if target.shape[1] == 1:
                    ...
                    y = torch.LongTensor(target.argmax(axis=1))
            ...
            # 🔧 TARGET FIX - CUDA assertion hatasını önler
            print("🔧 Fixing targets for binary classification...")
            print(f"📊 Original y_train: min={y_train.min()}, max={y_train.max()}")
            print(f"📊 Original y_train unique: {torch.unique(y_train)}")
            print(f"📊 Original y_val: min={y_val.min()}, max={y_val.max()}")
            print(f"📊 Original y_val unique: {torch.unique(y_val)}")

            # Clamp targets to [0, 1] for binary classification
            y_train = torch.clamp(y_train, 0, 1).long()
            y_val = torch.clamp(y_val, 0, 1).long()

            print(f"✅ Fixed y_train: min={y_train.min()}, max={y_train.max()}")
            print(f"✅ Fixed y_train unique: {torch.unique(y_train)}")
            print(f"✅ Fixed y_val: min={y_val.min()}, max={y_val.max()}")
            print(f"✅ Fixed y_val unique: {torch.unique(y_val)}")
            ...
            model_configs = trainer.generate_diverse_configs(
                base_configs, 
                args.ensemble_size, 
                args.model_diversity
            )
            
            print(f"🏋️ Training ensemble models for {pair_name}...")
            
            # 🔧 COMPLETELY FIXED: Method call with correct parameters
            models_results = trainer.train_diverse_models(
            ...
            if not models_results:
                print(f"❌ {pair_name}: No models trained successfully!")
                continue
            
            print(f"✅ {pair_name}: {len(models_results)} models trained successfully")
            
            # Evaluate ensemble
            evaluation = trainer.evaluate_ensemble(
                models_results, X_val, y_val, args.ensemble_strategy
            )
            
            ensemble_accuracy = evaluation['ensemble_accuracy']
            individual_accuracies = evaluation['individual_accuracies']
            best_individual = evaluation['best_individual']
            ...
            # Save ensemble
            save_dir = f"ensemble_models/{pair_name}"
            ...
            with open(csv_path, 'a') as f:
                f.write(f"{pair_name},{len(models_results)},{args.ensemble_strategy},{ensemble_accuracy:.4f},{best_individual:.4f},{improvement:.4f},{args.model_diversity}\n")
            
            print(f"💾 {pair_name} ensemble saved to {save_dir}")
            
        except Exception as e:
            print(f"❌ {pair_name} ensemble training failed: {e}")
            import traceback
            traceback.print_exc()
            continue
    
    print(f"\n🎉 Ensemble Training Complete!")
    print(f"📊 Detailed logs: {csv_path}")
