...
PDFHybridForexModel = None
create_pdf_hybrid_model = None
get_pdf_training_params = None
...
# PDF Hybrid Model için import
try:
    from .pdf_hybrid import PDFHybridForexModel, create_pdf_hybrid_model, get_pdf_training_params
    PDF_HYBRID_AVAILABLE = True
    logger.info("✅ PDF Hybrid model import başarılı")
except ImportError as e:
    logger.warning(f"⚠️ PDF Hybrid model import hatası: {e}")
    PDF_HYBRID_AVAILABLE = False
except Exception as e:
    logger.error(f"❌ Beklenmeyen hata (pdf_hybrid): {e}")
    PDF_HYBRID_AVAILABLE = False
...
if PDF_HYBRID_AVAILABLE:
    SUPPORTED_MODELS.extend(['pdf_hybrid', 'pdf_hybrid_lstm_transformer'])
    logger.info("✅ PDF Hybrid modeli desteklenen listeye eklendi")
...
PDF_HYBRID_ALIASES = ['pdf_hybrid', 'pdf_hybrid_lstm_transformer'] if PDF_HYBRID_AVAILABLE else []
...
if PDF_HYBRID_AVAILABLE and PDFHybridForexModel:
    available_types.append(PDFHybridForexModel)
...
def create_model(
    ...
    try:
        ...
        elif model_type_normalized in PDF_HYBRID_ALIASES:
            if not PDF_HYBRID_AVAILABLE or not PDFHybridForexModel:
                raise RuntimeError("PDF Hibrit modeli desteklenmiyor (import hatası)")
            
            logger.info("🔥 PDF Hibrit LSTM-Transformer modeli oluşturuluyor...")
            
            # PDF Hibrit spesifik validasyonlar
            validate_pdf_hybrid_config(config)
            
            # PDF optimal parametreler
            model_config = config.get('model', {})
            transformer_config = config.get('transformer', {})
            
            # PDF önerilen varsayılanlar
            pdf_defaults = {
                'd_model': 512,  # PDF sayfa 16 önerisi
                'nhead': 8,      # PDF sayfa 16 önerisi
                'num_layers': 4, # PDF sayfa 16 önerisi
                'dropout_rate': 0.15,  # PDF sayfa 18 önerisi
                'target_mode': 'binary'  # PDF binary sınıflandırma
            }
            
            # Kullanıcı konfigürasyonuyla birleştir
            for key, default_value in pdf_defaults.items():
                if key not in model_config and key not in transformer_config:
                    if key in ['d_model', 'nhead', 'num_layers']:
                        transformer_config[key] = default_value
                    else:
                        model_config[key] = default_value
            
            # PDF konfigürasyonunu logla
            logger.info(f"   📐 PDF Konfig: d_model={transformer_config.get('d_model', 512)}")
            logger.info(f"   📐 PDF Konfig: nhead={transformer_config.get('nhead', 8)}")
            logger.info(f"   📐 PDF Konfig: layers={transformer_config.get('num_layers', 4)}")
            logger.info(f"   🎯 Hedef modu: {model_config.get('target_mode', 'binary')}")
            
            # PDF Hibrit model oluştur
            model = create_pdf_hybrid_model(config, n_features, device)
            
            # Eğitim parametrelerini al
            training_params = get_pdf_training_params(model)
            logger.info(f"   📊 PDF Eğitim parametreleri: lr={training_params['learning_rate']}")
            logger.info(f"   📊 PDF Eğitim parametreleri: batch_size={training_params['batch_size']}")
            logger.info(f"   📊 PDF Eğitim parametreleri: scheduler={training_params['scheduler']}")
            
            # Eğitim parametrelerini modelde sakla
            model.recommended_training_params = training_params
            
            logger.info(f"   ✅ PDF Hibrit modeli başarıyla oluşturuldu")
            return model
...
def validate_pdf_hybrid_config(config: ModelConfig) -> None:
    """
    PDF Hybrid model konfigürasyonu doğrulama.
    
    Args:
        config: Model konfigürasyonu
        
    Raises:
        ValueError: Geçersiz konfigürasyon durumunda
    """
    model_config = config.get('model', {})
    transformer_config = config.get('transformer', {})
    ...
    # PDF spesifik validasyonlar
    if d_model % nhead != 0:
        raise ValueError(f"d_model ({d_model}) nhead ({nhead}) ile bölünebilir olmalı")
    
    if d_model < 256 or d_model > 1024:
        logger.warning(f"⚠️ PDF d_model için 256-1024 arasını önerir, girilen: {d_model}")
    
    if nhead not in [4, 8, 12, 16]:
        logger.warning(f"⚠️ PDF nhead için [4,8,12,16] değerlerini önerir, girilen: {nhead}")
    
    if num_layers < 2 or num_layers > 8:
        logger.warning(f"⚠️ PDF num_layers için 2-8 arasını önerir, girilen: {num_layers}")
    
    if dropout < 0.1 or dropout > 0.3:
        logger.warning(f"⚠️ PDF dropout için 0.1-0.3 arasını önerir, girilen: {dropout}")
    
    if target_mode not in ['binary', 'three_class']:
        raise ValueError(f"target_mode 'binary' veya 'three_class' olmalı, girilen: {target_mode}")
    
    logger.info("✅ PDF Hybrid konfigürasyon doğrulaması başarılı")
...
def suggest_training_params(model: ModelInstance) -> Dict[str, Any]:
    ...
    try:
        ...
        # PDF Hibrit model için öneriler
        if PDF_HYBRID_AVAILABLE and isinstance(model, PDFHybridForexModel):
            # PDF spesifik öneriler
            training_params = get_pdf_training_params(model)
            logger.info(f"   📊 PDF önerilen eğitim parametreleri: LR={training_params['learning_rate']}, BS={training_params['batch_size']}")
            return training_params
        
        # Hibrit model için öneriler
        elif HYBRID_MODEL_AVAILABLE and isinstance(model, HybridLSTMTransformer):
...
def validate_model_compatibility(model_type: str, config: ModelConfig) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        elif model_type.lower() in PDF_HYBRID_ALIASES:
            validate_pdf_hybrid_config(config) if PDF_HYBRID_AVAILABLE else warnings.append("PDF Hibrit model mevcut değil")
        else:
...
def test_pdf_hybrid_model_creation():
    """Test PDF Hybrid model oluşturma"""
    if not PDF_HYBRID_AVAILABLE:
        logger.warning("⚠️ PDF Hybrid model test atlandı - model mevcut değil")
        return False
    ...
    try:
        ...
        # Factory kullanarak model oluştur
        model = create_model('pdf_hybrid', test_config, n_features=82, device=device)
        ...
        assert output.shape == (batch_size, expected_output_size), f"Hatalı çıktı şekli: {output.shape}"
        
        logger.info(f"✅ PDF Hybrid factory testi başarılı!")
        logger.info(f"   Giriş: {test_input.shape} → Çıktı: {output.shape}")
        logger.info(f"   Parametreler: {sum(p.numel() for p in model.parameters()):,}")
        
        return True
        
    except Exception as e:
        logger.error(f"❌ PDF Hybrid factory testi başarısız: {e}")
        import traceback
        logger.error(f"Traceback: {traceback.format_exc()}")
        return False
...
__all__ = [
    ...
    'validate_pdf_hybrid_config' if PDF_HYBRID_AVAILABLE else 'validate_lstm_config',
    'get_model_complexity_score',
    'suggest_training_params',
    'get_supported_models',
    'validate_model_compatibility',
    'test_hybrid_model_creation',
    'test_pdf_hybrid_model_creation' if PDF_HYBRID_AVAILABLE else 'test_hybrid_model_creation',
    'SUPPORTED_MODELS',
    'ModelConfig',
    'TransformerConfig', 
    'LSTMConfig',
