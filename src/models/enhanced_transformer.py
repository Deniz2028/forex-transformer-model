...
class EnhancedTransformer(nn.Module):
    ...
    - GELU activation
    - FIXED: Binary classification outputs 2 classes for CrossEntropyLoss compatibility
    ...
    def __init__(
        ...
        # 🔧 FIXED: Output classifier for CrossEntropyLoss compatibility
        if target_mode == 'three_class':
            output_size = 3
        else:
            output_size = 2  # FIXED: Binary classification = 2 outputs for CrossEntropyLoss
        ...
        self.classifier = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(d_model // 4, output_size)  # Now outputs 2 for binary, 3 for three_class
        )
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None,
        ...
        """
        Enhanced Transformer forward pass with FIXED output handling.
        ...
        # 🔧 FIXED: Return raw logits for both binary and multi-class
        # Let the loss function handle activation (CrossEntropyLoss for both cases)
        output = logits  # Raw logits: [batch_size, 2] for binary, [batch_size, 3] for three_class
...
# Training function with FIXED loss function selection
def train_enhanced_transformer(model: EnhancedTransformer, X: np.ndarray, y: np.ndarray, 
    ...
    """
    Enhanced Transformer training function with PDF optimizations and FIXED loss handling.
    ...
    y_tensor = torch.LongTensor(y).to(device)  # FIXED: LongTensor for CrossEntropyLoss
    ...
    # 🔧 FIXED: Loss function - Always use CrossEntropyLoss for consistency
    criterion = nn.CrossEntropyLoss()
    logger.info(f"   🔥 Using CrossEntropyLoss for {model.target_mode} classification")
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_pbar):
            ...
            # Forward pass
            output = model(data)  # [batch_size, 2] for binary, [batch_size, 3] for three_class
            
            # 🔧 FIXED: Loss calculation with CrossEntropyLoss
            loss = criterion(output, target) / accumulation_steps
            pred = output.argmax(dim=1)  # Get class predictions
        ...
        with torch.no_grad():
            ...
            for data, target in val_pbar:
                ...
                # 🔧 FIXED: Validation loss and accuracy
